...
def test_n_bits_out_of_bounds_raises_error(tmp_path):
    with pytest.raises(ValueError, match="n_bits must be between 1 and 32"):
        generate(data="valid", output_path=str(tmp_path / "0.png"), n_bits=0, key=b"TEST")
        
    with pytest.raises(ValueError, match="n_bits must be between 1 and 32"):
        generate(data="valid", output_path=str(tmp_path / "100.png"), n_bits=100, key=b"TEST")
...
def test_str_key_matches_bytes_key(tmp_path):
    ...
    res_str = generate(
        data="test", output_path=str(tmp_path / "str.png"), 
        key="TEST_KEY", nonce=fixed_nonce
    )
    res_bytes = generate(
        data="test", output_path=str(tmp_path / "bytes.png"), 
        key=b"TEST_KEY", nonce=fixed_nonce
    )
...
def test_fixed_nonce_produces_identical_qr(tmp_path):
    ...
    res1 = generate(
        data="test", output_path=str(tmp_path / "1.png"), 
        key=b"TEST", nonce=fixed_nonce
    )
    res2 = generate(
        data="test", output_path=str(tmp_path / "2.png"), 
        key=b"TEST", nonce=fixed_nonce
    )
